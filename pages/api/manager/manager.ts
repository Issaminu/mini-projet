    ...
    case "POST":
      try {
        ...
        if(name!=userI.name){
          ...
          return res.status(400).json({message:"The entered name is invalid"})
          }
          if (name.length < 3 || name.length > 30) {
      return res.status(400).json({message:"Name must be >= 3 and <=30 characters"})
          }
        ...
        if (regexEmail.test(email) == false) {
          return res.status(400).json({message:"error\":\"The entered email address is not valid"});
        }
          ...
          if(password!=null||password!=""||password!=undefined){ 
            ...
            return res.status(400).json({message:"error\":\"The entered password is not valid"});
            }
